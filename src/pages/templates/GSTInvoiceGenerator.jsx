...
export default function GSTInvoiceGenerator() {
  ...
  const generatePDF = () => {
    ...
    doc.text("GST INVOICE", 105, 25, { align: "center" });

    // Invoice Info (Top Right)
    doc.setFontSize(10);
    doc.setFont("helvetica", "normal");
    doc.text(`Invoice #: ${invoiceData.invoiceNumber}`, 190, 20, { align: "right" });
    doc.text(`Date: ${invoiceData.date}`, 190, 27, { align: "right" });
    ...
    doc.text(`${symbol}${taxes.cgst.toFixed(2)}`, 190, finalY, { align: "right" });
    
    // SGST
    finalY += 7;
    doc.text(`SGST (${invoiceData.sgst}%):`, 140, finalY);
    doc.text(`${symbol}${taxes.sgst.toFixed(2)}`, 190, finalY, { align: "right" });
    ...
    const doc = new Document({
      sections: [{
        ...
        children: [
            ...
            rows: [
              new TableRow({
                ...
                children: [
                  ...
                  new TableCell({ shading: { fill: "0047AB" }, children: [new Paragraph({ children: [new TextRun({ text: "Rate", bold: true, color: "FFFFFF" })] })], alignment: AlignmentType.RIGHT }),
          ...
          // Totals
          new Paragraph({
            children: [new TextRun({ text: `Taxable Value: ${symbol}${subtotal.toFixed(2)}`, size: 20 })],
            alignment: AlignmentType.RIGHT,
          }),
          new Paragraph({
            children: [new TextRun({ text: `CGST (${invoiceData.cgst}%): ${symbol}${taxes.cgst.toFixed(2)}`, size: 20 })],
            alignment: AlignmentType.RIGHT,
          }),
          new Paragraph({
            children: [new TextRun({ text: `SGST (${invoiceData.sgst}%): ${symbol}${taxes.sgst.toFixed(2)}`, size: 20 })],
            alignment: AlignmentType.RIGHT,
          }),
          new Paragraph({
            children: [new TextRun({ text: `Grand Total: ${symbol}${total.toFixed(2)}`, bold: true, size: 24, color: "0047AB" })],
            alignment: AlignmentType.RIGHT,
          }),
          
          new Paragraph({ text: "", spacing: { before: 800 } }),
          new Paragraph({
            children: [new TextRun({ text: "Created with Dabby - Your AI Business Assistant", italic: true, color: "808080", size: 18 })],
            alignment: AlignmentType.RIGHT,
          }),
        ],
      }],
